...
void NUC_RX_Decoder(uint8_t *NUC_buf, NUC_DATA *nd, uint8_t dlen)
{
    for (uint8_t i = 0; i < dlen; i++)
    {
        if (NUC_buf[i] == 0x01)
        {
            nd->yaw   = ((int16_t)((NUC_buf[1 + i] << 8) | NUC_buf[2 + i])) * 100.f / 32767.f;
            nd->pitch = ((int16_t)((NUC_buf[3 + i] << 8) | NUC_buf[4 + i])) * 100.f / 32767.f;
            return;
        }
    }
}
...
void USART1_IRQHandler(void)
{
    ...
    else if (huart1.Instance->SR & UART_FLAG_IDLE)
    {
        ...
        if ((hdma_usart1_rx.Instance->CR & DMA_SxCR_CT) == RESET)
        {
            ...
            // {
            NUC_RX_Decoder(NUC_rx_buf[0], &NUC_data, NUC_FRAME_LENGTH);
            // }
        }
        else
        {
            ...
            // if (this_time_rx_len == NUC_FRAME_LENGTH)
            // {
            NUC_RX_Decoder(NUC_rx_buf[1], &NUC_data, NUC_FRAME_LENGTH);
            // }
        }
    }
