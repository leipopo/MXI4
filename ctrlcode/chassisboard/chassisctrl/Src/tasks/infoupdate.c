...
{
    ri->tar.yawangle += (-rcchannel_normalize(RC_Data.rc.ch[0]) + LIMIT(RC_Data.mouse.x,-10,10) ) / fre(infotaskperi) * yawspeedconst;
    ri->tar.pitangle += (rcchannel_normalize(RC_Data.rc.ch[1]) + LIMIT(RC_Data.mouse.y,-10,10)) / fre(infotaskperi) * pitspeedconst;
